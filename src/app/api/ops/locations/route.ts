...
import { dbOpsLocations } from "@/lib/db-ops";
...
export async function GET(request: NextRequest) {
  ...
  await initDb();
  const data = await dbOpsLocations.getAll(auth.payload.tenantId!);
...
export async function POST(request: NextRequest) {
  ...
  await dbOpsLocations.create({ id, tenantId: auth.payload.tenantId!, name, code, type, description, createdAt: now });

  logActivity({ tenantId: auth.payload.tenantId!, userId: auth.payload.userId, role: auth.payload.role,
    action: "created", entityType: "ops_location", entityId: id, entityName: name }).catch(() => {});
