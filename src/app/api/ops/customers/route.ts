...
import { dbOpsCustomers } from "@/lib/db-ops";
...
export async function GET(request: NextRequest) {
  ...
  await initDb();
  const data = await dbOpsCustomers.getAll(auth.payload.tenantId!);
...
export async function POST(request: NextRequest) {
  ...
  await dbOpsCustomers.create({ id, tenantId: auth.payload.tenantId!, name, code, contactName, contactEmail, contactPhone, address, notes, createdAt: now });

  logActivity({ tenantId: auth.payload.tenantId!, userId: auth.payload.userId, role: auth.payload.role,
    action: "created", entityType: "ops_customer", entityId: id, entityName: name }).catch(() => {});
